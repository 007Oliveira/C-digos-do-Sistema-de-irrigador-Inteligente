...
// Inicializa o LCD com o endereco do modulo e as dimensÃµes
LiquidCrystal_I2C lcd(0x27, 20, 4);

void setup() {
  Serial.begin(9600);              // Define a conexÃ£o serial em 9600 bits por segundo
  pinMode(pinoRele, OUTPUT);       // Define o pinoRele como saÃida
...
Serial.println();                           // Quebra uma linha no monitor serial
  ...
  lcd.setCursor(0, 1);             // Define o cursor no inicio da segunda linha do LCD
  ...
  if (Umid <= 45) {                // Se a umidade for menor ou igual a  45% faz...
    ...
    digitalWrite(pinoRele, LOW);   // Define a saida do pinoRele para nivel = LOW (Baixo), ligando a bomba

Serial.println("Irrigando...");               // Imprime no monitor serial "Irrigando..."
Serial.println("-------------------------");
Serial.println();                             // Quebra uma linha no monitor serial
    
  lcd.setCursor(0, 2);           // Define o cursor no inicio da terceira linha
  lcd.print("Irrigando...");     // Digita "Irrigando..." se a umidade for baixa
  } 

    else {
      
    digitalWrite(pinoRele, HIGH);  // Define a saÃ­da do pinoRele para nivel = HIGH (Alto), desligando a bomba
